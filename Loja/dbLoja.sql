...
default character set utf8; -- normas da lingua portuguesa
...
-- selecionar todos os registros ta tabela
select count(*) from tbl_livro;

-- selecionar todos os registros da tabela
select * from tbl_livro;

-- selecionar campos expecíficos da tabela
select no_isbn, nm_livro from tbl_livro;

-- selecionar outras tabelas
select * from tbl_autor;
select * from tbl_categoria;

-- inner join para vincular informações de outras tabelas e criando uma view
create view vw_livro
	as select tbl_livro.cd_livro, tbl_livro.no_isbn, tbl_categoria.ds_categoria, tbl_livro.nm_livro,
			tbl_autor.nm_autor, tbl_livro.no_pag, tbl_livro.vl_preco, tbl_livro.qt_estoque,
			tbl_livro.ds_resumo_obra, tbl_livro.ds_capa, tbl_livro.sg_lancamento
...
-- criando usuario
create user 'ead'@'localhost' identified with mysql_native_password by '3TH3ph4n1';
grant all privileges on dbloja.* to 'ead'@'localhost' with grant option;